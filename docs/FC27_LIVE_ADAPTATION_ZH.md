...
发布前验证：`npm run verify` 通过 244 文件 / 2,478 项测试；FC27 专项通过 40 文件 / 498 项及浏览器检查。已额外从 Git 暂存区干净检出，验证 Windows 换行转换后的 Runner/FSU 构建、SHA256 和发布门禁；默认门禁、`--packaging`、显式资产打包和旧 Profile 回归构建均通过。
