...
const paths = ['DailyLoopRunner.user.js', 'FSU_mod/fsu-mod.config.json', 'FSU_mod/fsu-mod-manifest.json'];
const assets = [];
for (const name of paths) {
  ...
  const expected = name === 'DailyLoopRunner.user.js'
    ? projectBaselineVersion(normalize(baseline), archivedVersion, currentVersion)
    : normalize(baseline);
  if (expected !== normalize(current)) throw new Error(`FC26 frozen input changed: ${name}`);
